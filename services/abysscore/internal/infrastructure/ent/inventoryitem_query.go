...
func (iiq *InventoryItemQuery) QueryUser() *UserQuery {
	query := (&UserClient{config: iiq.config}).Query()
...
func (iiq *InventoryItemQuery) QueryItem() *GameItemQuery {
	query := (&GameItemClient{config: iiq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := iiq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (iiq *InventoryItemQuery) Clone() *InventoryItemQuery {
	...
	return &InventoryItemQuery{
		...
		withUser:   iiq.withUser.Clone(),
		withItem:   iiq.withItem.Clone(),
...
func (iiq *InventoryItemQuery) WithUser(opts ...func(*UserQuery)) *InventoryItemQuery {
	query := (&UserClient{config: iiq.config}).Query()
...
func (iiq *InventoryItemQuery) WithItem(opts ...func(*GameItemQuery)) *InventoryItemQuery {
	query := (&GameItemClient{config: iiq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(inventoryitem.FieldUserID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.InventoryItem.Query().
//		Select(inventoryitem.FieldUserID).
//		Scan(ctx, &v)
...
func (iiq *InventoryItemQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*InventoryItem, error) {
	...
	if query := iiq.withUser; query != nil {
		if err := iiq.loadUser(ctx, query, nodes, nil,
			func(n *InventoryItem, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	if query := iiq.withItem; query != nil {
		if err := iiq.loadItem(ctx, query, nodes, nil,
			func(n *InventoryItem, e *GameItem) { n.Edges.Item = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (iiq *InventoryItemQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*InventoryItem, init func(*InventoryItem), assign func(*InventoryItem, *User)) error {
...
func (iiq *InventoryItemQuery) loadItem(ctx context.Context, query *GameItemQuery, nodes []*InventoryItem, init func(*InventoryItem), assign func(*InventoryItem, *GameItem)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*InventoryItem)
	for i := range nodes {
		fk := nodes[i].ItemID
...
func (iiq *InventoryItemQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := iiq.ctx.Fields; len(fields) > 0 {
		...
		if iiq.withUser != nil {
			_spec.Node.AddColumnOnce(inventoryitem.FieldUserID)
		}
		if iiq.withItem != nil {
			_spec.Node.AddColumnOnce(inventoryitem.FieldItemID)
		}
	}
