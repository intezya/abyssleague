...
func (uq *UserQuery) QueryStatistics() *StatisticQuery {
	query := (&StatisticClient{config: uq.config}).Query()
...
func (uq *UserQuery) QuerySentFriendRequests() *FriendRequestQuery {
	query := (&FriendRequestClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryReceivedFriendRequests() *FriendRequestQuery {
	query := (&FriendRequestClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
...
// QueryCurrentMatch chains the current query on the "current_match" edge.
func (uq *UserQuery) QueryCurrentMatch() *MatchQuery {
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withSentFriendRequests:     uq.withSentFriendRequests.Clone(),
		withReceivedFriendRequests: uq.withReceivedFriendRequests.Clone(),
...
func (uq *UserQuery) WithSentFriendRequests(opts ...func(*FriendRequestQuery)) *UserQuery {
	query := (&FriendRequestClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withSentFriendRequests = query
	return uq
}
...
func (uq *UserQuery) WithReceivedFriendRequests(opts ...func(*FriendRequestQuery)) *UserQuery {
	query := (&FriendRequestClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withReceivedFriendRequests = query
	return uq
}
...
func (uq *UserQuery) WithCurrentItem(opts ...func(*InventoryItemQuery)) *UserQuery {
	...
	uq.withCurrentItem = query
	return uq
}

// WithCurrentMatch tells the query-builder to eager-load the nodes that are connected to
// the "current_match" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithCurrentMatch(opts ...func(*MatchQuery)) *UserQuery {
	query := (&MatchClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withCurrentMatch = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	var (
		...
		loadedTypes = [8]bool{
			...
			uq.withReceivedFriendRequests != nil,
			uq.withItems != nil,
	...
	if query := uq.withStatistics; query != nil {
		if err := uq.loadStatistics(ctx, query, nodes,
			func(n *User) { n.Edges.Statistics = []*Statistic{} },
			func(n *User, e *Statistic) { n.Edges.Statistics = append(n.Edges.Statistics, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := uq.withSentFriendRequests; query != nil {
		if err := uq.loadSentFriendRequests(ctx, query, nodes,
	...
	if query := uq.withReceivedFriendRequests; query != nil {
		if err := uq.loadReceivedFriendRequests(ctx, query, nodes,
...
func (uq *UserQuery) loadStatistics(ctx context.Context, query *StatisticQuery, nodes []*User, init func(*User), assign func(*User, *Statistic)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(statistic.FieldUserID)
	}
	query.Where(predicate.Statistic(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.StatisticsColumn), fks...))
	}))
...
func (uq *UserQuery) loadSentFriendRequests(ctx context.Context, query *FriendRequestQuery, nodes []*User, init func(*User), assign func(*User, *FriendRequest)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (uq *UserQuery) loadReceivedFriendRequests(ctx context.Context, query *FriendRequestQuery, nodes []*User, init func(*User), assign func(*User, *FriendRequest)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (uq *UserQuery) loadCurrentItem(ctx context.Context, query *InventoryItemQuery, nodes []*User, init func(*User), assign func(*User, *InventoryItem)) error {
	...
	nodeids := make(map[int][]*User)
	for i := range nodes {
		if nodes[i].CurrentItemInProfileID == nil {
			continue
		}
		fk := *nodes[i].CurrentItemInProfileID
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "current_item_in_profile_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (uq *UserQuery) loadCurrentMatch(ctx context.Context, query *MatchQuery, nodes []*User, init func(*User), assign func(*User, *Match)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*User)
	for i := range nodes {
		if nodes[i].CurrentMatchID == nil {
			continue
		}
		fk := *nodes[i].CurrentMatchID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "current_match_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (uq *UserQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := uq.ctx.Fields; len(fields) > 0 {
		...
		if uq.withCurrentItem != nil {
			_spec.Node.AddColumnOnce(user.FieldCurrentItemInProfileID)
		}
		if uq.withCurrentMatch != nil {
			_spec.Node.AddColumnOnce(user.FieldCurrentMatchID)
		}
	}
