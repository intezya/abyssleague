...
// QueryPlayer1 chains the current query on the "player1" edge.
func (mq *MatchQuery) QueryPlayer1() *UserQuery {
...
// QueryPlayer2 chains the current query on the "player2" edge.
func (mq *MatchQuery) QueryPlayer2() *UserQuery {
	query := (&UserClient{config: mq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := mq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (mq *MatchQuery) Clone() *MatchQuery {
	...
	return &MatchQuery{
		...
		withPlayer1: mq.withPlayer1.Clone(),
		withPlayer2: mq.withPlayer2.Clone(),
		withResults: mq.withResults.Clone(),
...
// WithPlayer1 tells the query-builder to eager-load the nodes that are connected to
// the "player1" edge. The optional arguments are used to configure the query builder of the edge.
func (mq *MatchQuery) WithPlayer1(opts ...func(*UserQuery)) *MatchQuery {
	query := (&UserClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	mq.withPlayer1 = query
	return mq
}

// WithPlayer2 tells the query-builder to eager-load the nodes that are connected to
// the "player2" edge. The optional arguments are used to configure the query builder of the edge.
func (mq *MatchQuery) WithPlayer2(opts ...func(*UserQuery)) *MatchQuery {
	query := (&UserClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	mq.withPlayer2 = query
	return mq
}
...
func (mq *MatchQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Match, error) {
	...
	if query := mq.withPlayer1; query != nil {
		if err := mq.loadPlayer1(ctx, query, nodes, nil,
			func(n *Match, e *User) { n.Edges.Player1 = e }); err != nil {
			return nil, err
		}
	}
	if query := mq.withPlayer2; query != nil {
		if err := mq.loadPlayer2(ctx, query, nodes, nil,
			func(n *Match, e *User) { n.Edges.Player2 = e }); err != nil {
			return nil, err
		}
	}
...
func (mq *MatchQuery) loadPlayer1(ctx context.Context, query *UserQuery, nodes []*Match, init func(*Match), assign func(*Match, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Match)
	for i := range nodes {
		fk := nodes[i].Player1ID
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "player1_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (mq *MatchQuery) loadPlayer2(ctx context.Context, query *UserQuery, nodes []*Match, init func(*Match), assign func(*Match, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Match)
	for i := range nodes {
		fk := nodes[i].Player2ID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "player2_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (mq *MatchQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := mq.ctx.Fields; len(fields) > 0 {
		...
		if mq.withPlayer1 != nil {
			_spec.Node.AddColumnOnce(match.FieldPlayer1ID)
		}
		if mq.withPlayer2 != nil {
			_spec.Node.AddColumnOnce(match.FieldPlayer2ID)
		}
	}
